...
class Pendaftaran extends CI_Controller
{
    ...
    {                               //Constroktor
        parent::__construct();
        $this->load->model('PendaftaranModel'); //memanggil pendaftaran model
    }
...
*/
        if (isset($_POST['btn_daftar'])) { //apakah tombol daftar di klik
            $cek_nopendaftaran = $this->PendaftaranModel->cek_nopendaftaran(); // apakah ada no pendaftaran yang sama
            if ($cek_nopendaftaran == true) { // cek no pendaftaran
                $this->session->set_flashdata('pesan', 'No Pendaftaran sudah terdaftar di sistem!');
                redirect('pendaftaran/daftar');
            } else {
                $upload = $this->PendaftaranModel->upload_bukti('bukti_daftar'); //upload bukti
                if ($upload['result'] == 'success') { // kalau berhasil sukses
                    $this->PendaftaranModel->insert_pendaftaran($upload);
                    redirect('pendaftaran/daftar');
                } else { // kalau tidak pesan eror
                    $this->session->set_flashdata('pesan', $upload['error']);
                    redirect('pendaftaran/daftar');
                }
            }
        } else { // kalau tidak di klik buka halaman daftar
            $data['title'] = "Pendaftaran Pengguna | SIMDAWA-APP"; //me load halaman awal pendaftaran
